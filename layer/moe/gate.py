...
class TopKGating(nn.Module):
    ...
    def update_aux_statistics(self, probs, logits, gates):
        ...
        if self.aux_loss == 'mi':
            ...
            self.neg_H_e_given_x = self.neg_H_e_given_x + \
                (probs * log_prob).sum() / probs.size(0)
            self.count_layers += 1
        else:
    ...
    def forward(self, x):
        ...
        if self.gate_type in ['linear', 'mlp']:
            ...
            z = self.w_gate(x)
            logits = log_gmm_posterior(F.normalize(z, p=2, dim=-1),
                                       F.normalize(self.expert_centroids, p=2, dim=-1)) * self.temperature.exp()

        probs = torch.softmax(logits, dim=1)

        if self.training and (self.sample_topk > 0):
            _, top_km1_indices = probs.topk(
                self.top_k - self.sample_topk, dim=1)
