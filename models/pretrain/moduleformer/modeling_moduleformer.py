...
class ModuleFormerAttention(nn.Module):
    def __init__(self, config: ModuleFormerConfig):
        """
        ...
        self.register_buffer(
            "mask",
            torch.tril(torch.ones(self.context_length,
                       self.context_length, dtype=torch.int8))
        )
        self.register_buffer(
            "cum_weight",
            torch.tril(torch.ones(self.context_length,
    ...
    def forward(
        ...
        q = q.view(B, T, self.top_k, self.n_head,
                   self.head_size)  # (B, T, k, nh, hs)
        k = k.view(B, context_length, self.n_head,
                   self.head_size)  # (B, T, nh, hs)
        v = v.view(B, context_length, self.n_head,
                   self.head_size)  # (B, T, nh, hs)

        mask = torch.tril(torch.ones(context_length, context_length,
                          dtype=torch.int8, device=q.device))[context_length - T:, :]
        cum_weight = torch.tril(torch.ones(
            context_length, context_length, device=q.device), -1).type_as(q)
...
class ModuleFormerModel(ModuleFormerPreTrainedModel):
    def __init__(self, config: ModuleFormerConfig):
        ...
        self.h = nn.ModuleList([ModuleFormerBlock(config)
